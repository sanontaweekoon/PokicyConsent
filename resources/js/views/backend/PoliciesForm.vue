        ...
        <form class="container mx-auto">
            ...
            <div class="space-y-2">
                ...
                <div class="grid grid-rows-1">
                    <div class="flex my-5">
                        <label class="mr-3 text-nowrap font-medium">เวลาประกาศ: </label>
                                        ...
                                        <div class="flex flex-row">
                                            <div class="flex flex-col">
                                                <VueFlatpickr v-model="form.publish_date" :config="{
                                                    dateFormat: 'Y-m-d',
                                                    minDate: 'today'
                                                }" placeholder="YYYY-MM-DD" :disabled="announce !== 'scheduled'"
                                                    :key="'date-' + announce" class="px-3 py-2 mr-5 border rounded" />
...
function clearScheduleFields() {
    ...
    v$.value.publish_date?.$reset?.()
    v$.value.publish_time?.$reset?.()
}

watch(announce, async (val) => {
    if (val === 'now') {
        clearScheduleFields();
        await nextTick();
    }
...
async function saveDraft() {
    ...
    try {
        ...
        if (announce.value === "scheduled") {
            ...
            // ถ้าเลือก now ให้ validate แค่ title, category , description
            if (
                !form.value.title ||
        ...
        const message =
            announce.value === "scheduled"
                ? "บันทึกฉบับร่างพร้อมเวลาประกาศเรียบร้อยแล้ว"
...
function normalizeTime(value, withSeconds = false) {
    if (!value) {
        return '';
    }
    const s = String(value).trim();
...
async function loadPolicy(id) {
    ...
    const fromPublishAtDate = payload.publish_at ? normalizeDate(payload.publish_at) : '';
    const fromPublishAtTime = payload.publish_at ? normalizeTime(payload.publish_at) : '';
    ...
    function getPublishTs(p) {
        if (!p) {
            return NaN;
        }
        if (p.publish_at) {
            const t = Date.parse(String(p.publish_at));
            if (!isNaN(t)) {
                return t;
            }
        }
        if (p.publish_date && p.publish_time) {
            const t2 = Date.parse(`${p.publish_date} ${p.publish_time}`);
            if (!isNaN(t2)) {
                return t2;
            }
        }
    ...
    const ts = getPublishTs({
        publish_at: payload.publish_at,
        publish_date: payload.publish_date,
        publish_time: payload.publish_time,
    });

    const isFuture = !isNaN(ts) && ts > Date.now();
